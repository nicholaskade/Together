...
function Map({ pos, setMap, map }) {
  ...
  const mapSpinner = isLoading ? (
    <Spinner id="map-spinner" variant="light" />
  ) : (
    <div id="google-map" className="main-map" ref={ref} />
  );

  function mapLoading() {
    const mapLoad = setTimeout(() => {
      setIsLoading(false);
    }, 3500);
  }
  ...
  function fetchCoords() {
    ...
    fetch("/get_coords", postRequest).then((response) => {
      if (response.ok) {
  ...
  function handleSubmitDate() {
    ...
    fetch("/outings", postRequest).then((response) => {
      if (response.ok) {
        response.json().then((outing) =>
          dispatch({
            type: "add",
            date: outing,
          })
  ...
  return (
    <>
      <div id="dislikes-header">
        <h1 id="dates-title" onClick={() => navigate("/dates")}>
          Your Dates
        </h1>
        <AddButton
      ...
      <Modal show={showDateModal} onHide={handleCancel}>
        ...
        <Modal.Footer>
          ...
          <button className="together-button" onClick={() => fetchCoords()}>Confirm</button>
        </Modal.Footer>
      </Modal>
