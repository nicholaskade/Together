...
function UserProfile() {
  ...
  useEffect(() => {
    fetch(`/users/${profileUser}`).then((response) => {
      if (response.ok) {
  ...
  const friendIds = friends
    ? friends.map((friend) => {
        return friendIdArray.push(friend.id);
      })
    : [];
  ...
  function handleFriend() {
    ...
    fetch("/friendships", postRequest).then((response) => {
      if (response.ok) {
  ...
  function handleUnfriend() {
    ...
    fetch(`/user/${userId}/unfriend/${profileUser}`, deleteRequest).then(
      (response) => {
        if (response.ok) {
          response.json().then((friend) => {
            friendsDispatch({
    ...
    <div id="profile-header">
      ...
      {friends && profileUser !== userId ? (
        friendIdArray.includes(profileUser) ? (
          <button className="together-button" onClick={() => handleUnfriend()}>Unfriend</button>
        ) : (
          <button className="together-button" onClick={() => handleFriend()}>Add Friend</button>
        )
      ) : (
        <></>
      )}
