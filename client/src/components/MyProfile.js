...
import SignificantOtherSidebar from "./SignificantOtherSidebar";
...
function MyProfile() {
  ...
  function handleSubmitEdits() {
    ...
    fetch(`/users/${currentUser.id}`, patchRequest).then((res) => {
      if (res.ok) {
  ...
  const renderProfile = user ? (
    <div id="profile-header">
      ...
      <div id="profile-identifiers">
        ...
        <p>
          {user.gender} • {user.relationship_status}
        </p>
      </div>
      <button className="together-button" onClick={() => clickEdit()}>
        edit profile
      </button>
    </div>
  ) : errors.length > 0 ? (
  ...
  return (
    <>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        ...
        <Modal.Footer>
          <button className="together-button" onClick={() => setShowEditModal(false)}>Cancel</button>
          <button className="together-button" onClick={() => handleConfirm()}>Confirm</button>
        </Modal.Footer>
      </Modal>
