...
function SignificantOtherSidebar() {
    ...
    const renderFriends =
        friends && friends.length > 0 ?
            notSos.map((friend) => {
                return (
                    <div className="search-default-card" onClick={(e) => handleSoSelection(friend.id)}>
                        <img src={friend.profile_picture} alt={friend.full_name} className="friend-button" onClick={(e) => handleSoSelection(friend.id)}/>
                        <div className="search-card-names" onClick={(e) => handleSoSelection(friend.id)}>
                            <h6 onClick={(e) => handleSoSelection(friend.id)}>{friend.full_name}</h6>
                            <h6 onClick={(e) => handleSoSelection(friend.id)}>{friend.username}</h6>
                        </div>
                    </div>
                )
            })
    ...
    function handleCreateSo() {
        fetch(`/user/${uid}/make_significant/${selectedFriend.id}`)
        .then(res => {
            if (res.ok) {
                res.json().then(friend => setSignificantOthers([...significantOthers, friend]));
            } else {
                res.json().then(errors => setErrors(errors));
            }
        });
            ...
            <>
                <div id="significant-other-sidebar-container">
                    <div id="significant-other-sidebar-header">
                        ...
                        <AddButton className="sidebar-button" onClick={() => handleAddClick()}/>
                    </div>
                ...
                <Modal show={showAddSoModal} onHide={handleCloseSoModal}>
                    ...
                    <Modal.Footer>
                        <button onClick={() => handleCloseSoModal()}>Cancel</button>
                    </Modal.Footer>
                </Modal>
                ...
                <Modal show={showConfirmationModal} onHide={handleCloseConfirmationModal}>
                    ...
                    <Modal.Body>
                        {
                            selectedFriend ?
                                <p>Would you like to make {selectedFriend.full_name} a significant other?</p>
                                    : <></>
                        }
                    </Modal.Body>
                    ...
                    <Modal.Footer>
                        ...
                        <button onClick={() => handleCreateSo()}>Confirm</button>
                    </Modal.Footer>
                </Modal>
            </>
        );
