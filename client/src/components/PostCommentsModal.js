...
function PostCommentsModal({
  ...
  const isAdmin = useUser().user.is_admin;

  useEffect(() => {
    if (targetPost) {
      fetch(`/post/${targetPost}/comments`).then((res) => {
        if (res.ok) {
    ...
    if (targetPost) {
      fetch(`/posts/${targetPost}`).then((res) => {
        if (res.ok) {
  ...
  function handleCommentDelete(comment_id) {
    ...
    fetch(`/comments/${comment_id}`, deleteRequest).then((res) => {
      if (res.ok) {
        res
          .json()
          .then((deletedComment) =>
  ...
  function handleSubmitComment(e) {
    ...
    fetch("/comments", postRequest).then((res) => {
      if (res.ok) {
        res.json().then((comment) => setComments([...comments, comment]));
      } else {
        res.json().then((errors) => setErrors(errors));
      }
    });
      ...
      <div className="post-footer">
        ...
        {likedPosts ? (
          likedPosts.includes(post.id) ? (
            <LikedButton
              onClick={(e) => handleUnlike(post.id)}
